...
Route::middleware("language")->name("admin.")->group(function () {
    ...
    Route::middleware("auth:web")->group(function () {
        ...
        Route::get('bank/upload', "IndexController@bankUpload")->name('bank.upload');
        ...
        Route::middleware(["rbac.admin"])->group(function () {
            ...
            Route::get('member/online', 'MembersController@member_online')->name('member.member_online');
            ...
            Route::get("apis/wallet", "ApisController@wallet")->name('apis.wallet');
            Route::resource("apis", "ApisController")->except('show');

            Route::get('apigames/category', 'ApiGamesController@category')->name('apigames.mobile_category');
            ...
            Route::get('gamerecords/report', 'GameRecordsController@report')->name('gamerecords.report');
            Route::get('gamerecords/report/detail/{member_id}', 'GameRecordsController@reportDetail')->name('gamerecords.report_detail');
            Route::get('gamerecords/betting', 'GameRecordsController@betting')->name('gamerecords.betting');
            Route::get('gamerecords/betting/detail/{member_id}', 'GameRecordsController@bettingDetail')->name('gamerecords.betting_detail');
            Route::get('gamerecords/getBetDetail/{id}', 'GameRecordsController@getBetDetail')->name('gamerecords.getBetDetail');
            Route::get('gamerecords/getBetDetail/Swmd/{id}', 'GameRecordsController@getBetDetailSwmd')->name('gamerecords.getBetDetailSwmd');
            Route::get('gamerecords/pullReportByWagerId/Swmd/{id}', 'GameRecordsController@pullReportByWagerId')->name('gamerecords.pullReportByWagerId');
            
            Route::resource("gamerecords", "GameRecordsController")->only(['index', 'destroy', 'show']);
            Route::get("gamerecord/report/export-sbo/{id}", "GameRecordsController@exportSbo")->name("gamerecord.export-sbo");
            ...
            Route::get('casso', 'CassoController@index')->name('casso.index');

            Route::resource("eeziepays", "EeziepaysController")->only(['index', 'show']);

            Route::resource("publishers", "PublisherController");

            Route::resource("memberwheelawards", "MemberWheelAwardController");

            Route::get("statements", "StatementController@index")->name('statements.index');

            Route::get("transactionStatistics", "TransactionStatisticController@index")->name('transactionStatistics.index');

            Route::group(['prefix'=>'seamless','as'=>'seamless.'], function(){
                Route::get('/launch-game', "GameSwmdController@launchGame")->name('launch_game');
                Route::get('/pull-report', "GameSwmdController@pullReport")->name('pull_report');
            });
        });
    });
});
