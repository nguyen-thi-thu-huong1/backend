...
if (!function_exists('public_url')) {
    function public_url($url)
    {
        ...
        return $appURL . trim($url, '/');
    }
}

if (!function_exists('moneyFormat')) {
    function moneyFormat($money, $decimals = 2)
    {
...
if (!function_exists('getCurrentCurrency')) {
    ...
    {
        $currentLanguage = request()->user('web')->lang ?? session('applocale', 'zh_cn');

        return getConfig('currency.' . $currentLanguage);
    }
}
