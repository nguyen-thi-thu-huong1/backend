...
class Member extends Authenticatable implements JWTSubject
{
    ...
    public function getSboHistories($params = array())
    {
        ...
        ->with(['sboRecords' => function ($query) use ($from, $to, $productType, $timezone) {
            $query->whereIn('status', [TransactionHistory::STATUS_WIN, TransactionHistory::STATUS_LOST, TransactionHistory::STATUS_TIE, TransactionHistory::STATUS_CANCEL])
                  ->when($from && $to, function ($query1) use ($from, $to, $timezone) {
                      $query1->whereBetween($timezone, [$from, $to]);
                  })
                  ->when($productType, function ($query3) use ($productType) {
                      $query3->where('product_type', $productType);
                  });
        }])
        ->whereHas('sboRecords', function ($q) use ($from, $to, $productType, $timezone) {
            $q->whereIn('status', [TransactionHistory::STATUS_WIN, TransactionHistory::STATUS_LOST, TransactionHistory::STATUS_TIE])
    ...
    public function getBettingSboHistories($params = array())
    {
        ...
        $query = $this->when($memberName, function ($query) use ($memberName) {
            ...
            ->whereHas('sboRecords', function ($q) use ($from, $to, $productType) {
                $q->whereIn('status', [TransactionHistory::STATUS_WAITING]);
