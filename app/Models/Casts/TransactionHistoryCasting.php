...
trait TransactionHistoryCasting
{
    ...
    {
        return $this->member ? $this->member->phone : '';
    }
    ...
    public function getStatusText($import = false)
    {
        $class = data_get([
    ...
    public function getFsDetailText()
    {
        ...
        return 'Rate : '.$fsRate . ' Ngày : ' .$fsAt;
    }

    public function getFsDetailByMoneyText($money, $import = false)
    {
        if (!$this->is_fs) {
            return 'Chưa nhận';
        }
        ...
        if ($import) {
            return ($fsRate * $money / 100) . ' - ' . 'Rate : '.$fsRate . ' Ngày : ' .$fsAt;
        }
        return ($fsRate * $money / 100) .
        '<br/>'.
        'Rate : '.$fsRate . ' Ngày : ' .$fsAt;
    }
}
