...
class SendAgentController extends AdminBaseController
{
    ...
    {
        if (!$request->get('ids')) {
            return $this->failed(trans('res.base.item_select_required'));
        }

        if (!$request->get('created_at')) {
            return $this->failed(trans('res.agent_yj_log.msg.time_range_required'));
        }
        ...
        try {
            DB::transaction(function () use ($data, $month) {
                foreach ($data['ids'] as $member_id) {
                    if (floatval($data['yl_money'][$member_id]) < 0) {
                        continue;
                    }
