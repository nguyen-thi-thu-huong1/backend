...
class ApisController extends AdminBaseController
{
    ...
    {
        $data = $this->model->get();
        return view("{$this->view_folder}.wallet", compact('data'));
    }
    ...
    public function handleExceptResponse($models) {
        switch (request()['api-page-current']) {
            case 'api-wallet':
                return route("{$this->root_folder}.{$models}.wallet");
            default:
                return;
