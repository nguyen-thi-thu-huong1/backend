...
class SystemConfigsController extends AdminBaseController
{
    ...
    public function updateConfigsByName(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $arr = explode('-', $key);
            // check config exist
            $item = $this->model->where('name', $arr[0])->where('lang', $arr[1])->first();
            // if not exist config, it will be create new record
            if (blank($item)) {
                $item = $this->model;
                $item->name = $arr[0];
                $item->title = $arr[0];
                $item->config_group = 'remote_api';
                $item->lang = $arr[1];
            }
            $item->value = $value;
            $item->save();
        ...
        // update agent bet setting
        if (!empty(data_get($data, 'agent_min_bet_setting-common'))
            && !empty(data_get($data, 'agent_max_bet_setting-common'))
            && !empty(data_get($data, 'agent_max_per_match-common'))
        ) {
            $this->updateBetSetting($data);
        }
    ...
    protected function updateBetSetting($configs)
    {
        try {
            $params = [
                'CompanyKey' => data_get($configs, 'company_key-common'),
                'ServerId' => data_get($configs, 'server_id-common'),
                'Username' => getConfig('sbo_agent'),
                'min' => intval(data_get($configs, 'agent_min_bet_setting-common')),
                'max' => intval(data_get($configs, 'agent_max_bet_setting-common')),
                'MaxPerMatch' => intval(data_get($configs, 'agent_max_per_match-common')),
                'CasinoTableLimit' => intval(data_get($configs, 'casino_table_limit-common')),
            ];
