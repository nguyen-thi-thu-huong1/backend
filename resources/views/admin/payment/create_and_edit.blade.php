        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="post" class="form-horizontal"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <select name="type" class="form-control js_select2">
                                ...
                                @foreach(trans('res.option.payment_type') as $key =>$value)
                    ...
                    <div class="form-group" id="cardpay">
                        <label class="col-sm-2 control-label required">@lang('res.payment.field.cardpay')</label>
                        <div class="col-sm-4">
                            <input type="text" class="form-control" name="params[token]"
                                       value="{{ $isUpdated && is_array($model->params) && array_key_exists('token',$model->params) ? $model->params['token'] : '' }}">
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="number" class="form-control" name="params[usdt_rate]"
                                       ...
                                       value="{{ isset($usdtRate) ? $usdtRate : ($isUpdated && is_array($model->params) && array_key_exists('usdt_rate',$model->params) ? $model->params['usdt_rate'] : '') }}"
                                       >
                            </div>
                        </div>
                    </div>
                    <div id="bankpay">
                        ...
                        <div class="form-group">
                            <label class="col-sm-2 control-label">@lang('res.payment.field.acqid')</label>
                            <div class="col-sm-4">
                                <input type="text" class="form-control" name="params[acqid]"
                                       value="{{ $isUpdated && is_array($model->params)  && array_key_exists('acqid',$model->params) ? $model->params['acqid'] : '' }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">@lang('res.payment.field.arrange')</label>
                        <div class="col-sm-2">
                            <input type="number" min="0" class="form-control" name="arrange" value="{{ $isUpdated?$model->arrange:"" }}" @if(!$isUpdated) required @endif>
                        </div>
                        <div class="col-sm-4 help-block">
                            @lang('res.payment.edit.notice_arrange')
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">@lang('res.payment.field.callback_url')</label>
                        <div class="col-sm-4">
                            <input type="text" class="form-control" name="callback_url" value="{{ $isUpdated?$model->callback_url:"" }}" @if(!$isUpdated) required @endif>
                        </div>
                        <div class="col-sm-2 help-block">
                            @lang('res.payment.edit.notice_callback_url')
                        </div>
                    </div>
                    <div class="form-group">
    ...
    <script>
        $(function () {
            ...
            function initView(){
                ...
                $('#cardpay').hide().find('input[name]').attr("disabled", true);
                ...
                $('#acqid').hide().find('input[name]').attr("disabled", true);
                ...
                if(typeSelectValue.indexOf('{{ \App\Models\Payment::PREFIX_THIRDPAY }}') == 0){
                    if(typeSelectValue == '{{ \App\Models\Payment::TYPE_CARDPAY }}'){
                        $('#cardpay').show().find('input[name]').attr("disabled", false);
                        $('#thirdpay').show().find('input[name]').attr("disabled", false);
                    }else if(
                        typeSelectValue == '{{ \App\Models\Payment::ONLINE_MOMO }}'
                        || typeSelectValue == '{{ \App\Models\Payment::ONLINE_ZALOPAY }}'
                        || typeSelectValue == '{{ \App\Models\Payment::ONLINE_VIETTELPAY }}'
                    ){
                    ...
                    $('#account').show().find('input[name]').attr("disabled", false);
                    $('#name').show().find('input[name]').attr("disabled", false);
                    $('#acqid').show().find('input[name]').attr("disabled", false);
