                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-6">
                            @foreach ($roles as $role)
                                <input type="checkbox" class="checkbox-primary" value="{{ $role->id }}" name="roles[]" @if(in_array($role->id,$user->roles->pluck("id")->toArray())) checked="checked" @endif style="margin-right: 5px;">{{ $role->name}}
                            @endforeach
                        </div>
                    </div>
