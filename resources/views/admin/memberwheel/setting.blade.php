...
@section('content')
    ...
    @php
        $lang = [request('currency',\App\Models\Base::LANG_CN), \App\Models\Base::LANG_COMMON];
        $config = [];
        $wheel_awards = \App\Models\MemberWheelAward::whereIn('lang', $lang)->where('is_open', true)->get()->toArray();
        foreach($wheel_awards as $k => $v){
            array_push($config, ['id' => $v['id'],'text' => $v['title']]);
        }
    @endphp
@endsection
