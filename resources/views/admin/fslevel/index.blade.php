@extends('layouts.baseframe')

@section('title', $_title)

@section('content')
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header">
                <h4>{{ $_title }}</h4>
                <ul class="card-actions">
                    <li>
                        <button type="button" data-toggle="collapse" href="#searchContent" aria-expanded="false" aria-controls="searchContent"><i class="mdi mdi-chevron-double-down"></i> @lang('res.btn.collapse')</button>
                    </li>
                    <li>
                        <button type="button" onclick="window.location.reload()"><i class="mdi mdi-refresh"></i> @lang('res.btn.refresh')</button>
                    </li>
                </ul>
            </div>
            <div class="card-body collapse in" id="searchContent" aria-expanded="true">
                <form action="" method="get" id="searchForm" name="searchForm">
                    <div class="row">
                        @include('layouts._search_field',
                        [
                        'list' => [
                            'created_at' => ['name' => trans('res.common.created_at'),'type' => 'datetime'],
                            'game_type' => ['name' => trans('res.common.game_type'),'type' => 'select','data' => trans('res.option.game_type')],
                            'member_id' => ['name' => trans('res.common.member_name'),'type' => 'select','data' => \App\Models\Member::getMemberArray()],
                            'lang' => ['name' => trans('res.common.lang'),'type' => 'select','data' => config('platform.lang_select')],
                            ]
                        ])
                    </div>
                    <div class="clearfix">
                        <div class="input-group">
                            <button type="submit" class="btn btn-primary">@lang('res.btn.search')</button>&nbsp;
                            <button type="reset" class="btn btn-warning"
                                    onclick="document.searchForm.reset()">@lang('res.btn.reset')</button>&nbsp;
                        </div>
                    </div>
                </form>
            </div>
        </div>

        <div class="card">
            <div class="card-toolbar clearfix">
                <div class="toolbar-btn-action">
                    <a class="btn btn-dark m-r-5" href="{{ route('admin.membermoneylogs.index', ['operate_type' => 4]) }}"><i class="mdi mdi-history"></i>@lang('res.btn.fs_history')</a>

                    <a class="btn btn-primary m-r-5" href="{{ route("admin.fslevels.create") }}"><i class="mdi mdi-plus"></i>@lang('res.btn.add')</a>

                    <a href="{{ route("admin.fslevels.batch_create") }}" class="btn btn-warning"><i class="mdi mdi-plus"></i>@lang('res.btn.batch_add')</a>

                    <a class="btn btn-danger" id="batchDelete" data-operate="delete" data-url="/admin/fslevels/ids"><i class="mdi mdi-window-close"></i> @lang('res.btn.delete')</a>
                </div>
            </div>

            <div class="card-body">
                @include('layouts._per_page')

                @include('layouts._paging')

                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                        <tr>
                            <th style="min-width: 50px;">
                                <label class="lyear-checkbox checkbox-primary">
                                    <input type="checkbox" id="check-all"><span></span>
                                </label>
                            </th>
                            <th>@lang('res.common.member_name')</th>
                            @include('layouts._table_header', ['data' => \App\Models\FsLevel::$list_field, 'model' => 'fs_level'])
                            <th>@lang('res.common.operate')</th>
                        </tr>
                        </thead>

                        <tbody>
                        @foreach ($data as $item)
                            <tr>
                                <td>
                                    <label class="lyear-checkbox checkbox-primary">
                                        <input type="checkbox" name="ids[]" value="{{ $item->id }}"><span></span>
                                    </label>
                                </td>
                                <td>
                                    @if($item->type == \App\Models\FsLevel::TYPE_SYSTEM)

                                    @else
                                        {{ $item->member->name ?? '' }}
                                    @endif
                                </td>
                                @include('layouts._table_body', ['data' => \App\Models\FsLevel::$list_field, 'item' => $item])
                                <td>
                                    <div class="btn-group">
                                        <a class="btn btn-xs btn-default"
                                           href="{{ route('admin.fslevels.edit',['fslevel' => $item->id]) }}" title=""
                                           data-toggle="tooltip" data-original-title="@lang('res.btn.edit')"><i
                                                    class="mdi mdi-pencil"></i></a>

                                        <a class="btn btn-xs btn-default" href="javascript:;" data-operate="delete"
                                           data-toggle="tooltip" data-original-title="@lang('res.btn.delete')"
                                           data-url="{{ route('admin.fslevels.destroy', ['fslevel' => $item->id]) }}">
                                            <i class="mdi mdi-window-close"></i>
                                        </a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                </div>

                @include('layouts._paging')
            </div>
        </div>
    </div>
@endsection
