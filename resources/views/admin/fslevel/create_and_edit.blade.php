            ...
            <div class="card-body">
                ...
                <form method="post" class="form-horizontal"
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">@lang('res.common.sbo_fs_type')</label>
                        <div class="col-sm-4">
                            <input type="checkbox" id="sbo_fs_type" name="sbo_fs_type" value="1" {{ $isUpdated && empty($model->game_type) ? 'checked' : '' }}>
                        </div>
                    </div>

                    <div class="form-group game_type {{ empty($model->game_type) ? 'hide' : '' }}">
                    ...
                    <div class="form-group product_type {{ empty($model->product_type) ? 'hide' : '' }}">
                        <label class="col-sm-2 control-label required">@lang('res.common.product_type')</label>
                        <div class="col-sm-4">
                            <select name="product_type" class="form-control js_select2">
                                <option value="">@lang('res.common.select_default')</option>
                                @foreach(\App\Models\TransactionHistory::getProductType() as $key =>$value)
                                <option value="{{ $key }}" @if($isUpdated && $model->product_type == $key) selected @endif>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
@section('footer-js')
    <script>
        $(function(){
            ...
            $('[name="sbo_fs_type"]').click(function () {
                triggerSboFsType();
            });
            ...
            function triggerSboFsType() {
                if ($('[name="sbo_fs_type"]').is(':checked')) {
