...
@foreach ($data as $key => $item)
    ...
    @switch($key)
        @case('member_id')
            <th style="min-width: 50px;">@lang('res.common.member_id')</th>
            @break
        @case('user_id')
            <th style="min-width: 50px;">@lang('res.common.user_id')</th>
            @break
        @case('agent_id')
            <th style="min-width: 50px;">@lang('res.common.api_id')</th>
            @break
        @default
        <th title="{{ $item['name'] }}"
